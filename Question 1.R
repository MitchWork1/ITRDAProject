...
#Splitting file path into two for "neatness"
file_path_1 <- "C:/Users/mitch/OneDrive - Eduvos/School/Year Three/Block 1"
file_path_2 <- "/ITDRA3-11/Assignment/graduate_survey.csv"

data_file_pAth <- paste(file_path_1, file_path_2, sep="")
                          ...
                          is_other = TRUE, include_other = TRUE)
{
  ...
  if(!include_other)
  {
    top_column <- column_counts |> slice_max(Freq, n = top_num)
  }
  ...
  else if(top_num < nrow(column_counts))
  {
    ...
    top_column <- column_counts |> slice_max(Freq, n = top_num -1)
  }
  else
  {
...
ui <- dashboardPage(
  dashboardHeader(title = "Top Technologies Used by Graduates"),
  dashboardSidebar(
    div(style = "text-align: center;", h4("Global Graph Settings")),
      selectInput("axisSize", "Axis Size", choices = seq(10, 18, by = 1),
                  selected = 10, width = "100%"),
      selectInput("legendSize", "Legend Size", choices = seq(2, 12, by = 1),
                  selected = 4, width = "100%"),
      selectInput("percentageSize", "Percentage Size",
                  choices = seq(2, 10, by = 1), selected = 4, width = "100%"),
